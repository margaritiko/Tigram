...
class SaveOperation: Operation {
    ...
    override func main() {
        UserService.getInstance().saveGivenUser(user: user, completion: { (success) in
...
class LoadOperation: Operation {
    ...
    override func main() {
        UserService.getInstance().getCurrentUser(completion: { (user) in
            // Serial Main queue
...
class OperationDataService: DataServiceProtocol {
